...
通过已有的 `window.electronAPI.sevenapp.request({ action: 'chat', request_id, messages, stream: true })` 调用 `resources/sevenapp/openai.py`，沿用其模型配置。浏览器中可预览布局，发送需要 Electron preload 桥接。

主进程为增量和完成响应附加 `request_id`；渲染层订阅后再发起请求，只接收当前请求的事件。`window.electronAPI.sevenapp.cancel(requestId)` 校验窗口归属，取消排队请求或结束正在执行该请求的 Python 进程；后续请求自动启动新进程，旧进程的迟到事件不会污染新请求。
